...
class Exp2:
    ...
    def train(self, model, labeled_loader_S, labeled_loader_T, unlabeled_loader_S, unlabeled_loader_T, optimizer,
        ...
        for batch_index in range(len(unlabeled_loader_S)):
            ...
            S_unlabeled, T_unlabeled, S_out_unlabeled, T_out_unlabeled, S_loss_se_unlabeled, T_loss_hse_unlabeled = model(
                unlabeled_batch_S, unlabeled_batch_T)
            ...
            loss_fix = model.loss_fix(S_out_unlabeled, T_out_unlabeled, self.configs["T2"], self.configs["threshold"])
            loss_simlr = model.loss_simlr(S_unlabeled, T_unlabeled)

            S_labeled, T_labeled, S_out_labeled, T_out_labeled, S_loss_se_labeled, T_loss_hse_labeled = model(
                labeled_batch_S, labeled_batch_T)
            ...
            if epoch > self.configs["warm_epochs"]:
                loss = loss_sup \
                       ...
                       + self.configs["weight_fix"] * loss_fix \
                       + self.configs["weight_simlr"] * loss_simlr
    ...
    def run(self, dataset, seed, device):
        self.fix_seed(seed)
    ...
    def exp_fold(self):
        ...
        for fold, (labeled_train_idx, unlabeled_train_idx, val_idx, test_idx) in enumerate(
            ...
            if fold >= self.configs["runs"]:
                break

            labeled_loader_S = IterLoader(
                DataLoader(dataset_S[labeled_train_idx], batch_size=int(np.ceil(self.configs["batch_size"] / 5)), shuffle=False))
            unlabeled_loader_S = IterLoader(
                DataLoader(dataset_S[unlabeled_train_idx], batch_size=self.configs["batch_size"], shuffle=False))
            val_loader_S = IterLoader(
                DataLoader(dataset_S[val_idx], batch_size=self.configs["batch_size"], shuffle=False))
            test_loader_S = IterLoader(
                DataLoader(dataset_S[test_idx], batch_size=self.configs["batch_size"], shuffle=False))

            labeled_loader_T = IterLoader(
                DataLoader(dataset_T[labeled_train_idx], batch_size=int(np.ceil(self.configs["batch_size"] / 5)), shuffle=False))
            unlabeled_loader_T = IterLoader(
                DataLoader(dataset_T[unlabeled_train_idx], batch_size=self.configs["batch_size"], shuffle=False))
            val_loader_T = IterLoader(
                DataLoader(dataset_T[val_idx], batch_size=self.configs["batch_size"], shuffle=False))
            test_loader_T = IterLoader(
                DataLoader(dataset_T[test_idx], batch_size=self.configs["batch_size"], shuffle=False))
            ...
            model = HypSEE2(in_channels=dataset.num_features,
                            ...
                            num_edges2=self.configs["num_edges2"],
                            avg_num_nodes=avg_num_nodes,
                            height=self.configs["height"]).to(device)
            for key in model.hyper_hierarchical_GRL.hyperconv_dict.keys():
                model.hyper_hierarchical_GRL.hyperconv_dict[key].to(device)
            for key in model.hyper_hierarchical_GRL.pool_dict.keys():
                model.hyper_hierarchical_GRL.pool_dict[key].to(device)
            ...
            for epoch in range(1, self.configs["epochs"] + 1):
                train_loss, train_loss_sup, train_loss_con, train_loss_se = self.train(model, labeled_loader_S,
                                                                                       labeled_loader_T,
                                                                                       unlabeled_loader_S,
                                                                                       unlabeled_loader_T,
                                                                                       optimizer, anchor_queue, epoch,
                                                                                       device)
                val_acc, val_loss_sup, val_loss_se = self.test(model, val_loader_S, val_loader_T, device)
                test_acc, _, _ = self.test(model, test_loader_S, test_loader_T, device)
                print(
            ...
            print("run: {}\tacc: {}\t".format(fold, best_test_acc))
    ...
    def exp(self):
        ...
        test_acc_list = []
        for repeat in range(self.configs["runs"]):
            print(repeat)
            # with torch.autograd.detect_anomaly(True):
            best_acc = self.run(dataset, repeat, device)
