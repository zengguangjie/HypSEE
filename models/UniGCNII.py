...
class UniGCNIIConv(MessagePassing):
    ...
    def forward(self, X, hyperedge_index, alpha, beta, X0, degV, degE, aug_weight = None, give_edge = False, edge_entry_weight = None) :
        ...
        if give_edge : 
        ...
        elif edge_entry_weight is not None:
            ...
            Xev = Xe[edges]  # [nnz, C]
            Xv = scatter(Xev * w.view(-1, 1), vertex, dim=0, reduce='sum', dim_size=N)  # [N, C]
            Xv = Xv * degV
...
class UniGATConv(nn.Module):
    ...
    def forward(self, X, hyperedge_index, edge_entry_weight=None):
        """edge_entry_weight: 可选 [nnz]，每个 (node, edge) 关联条目的权重（非负）。
        统一权重时与无权版本完全等价：边特征用加权平均，注意力 logits 加 log(w)
        （softmax 对整体缩放不变；w→0 的条目注意力趋于 0）。"""
        vertex = hyperedge_index[0]
        edges = hyperedge_index[1]
        ...
        if w is not None:
            alpha = alpha + torch.log(w).unsqueeze(-1)
...
def get_weighted_degree_of_hypergraph(hyperedge_index, w):
    """加权超图的度归一化因子，结构与 get_degree_of_hypergraph 一致，用关联权重 w 替代全 1。"""
    dV = scatter(w, hyperedge_index[0], dim=0, reduce='sum')
    dE = scatter(dV[hyperedge_index[0]], hyperedge_index[1], dim=0, reduce='mean')
    dV = dV.pow(-0.5)
    dE = dE.pow(-0.5)
    dV[dV.isinf()] = 1
    dE[dE.isinf()] = 1
...
def get_edge_features(X, hyperedge_index, degE, aug_weight = None, give_edge = False) :
    if give_edge or aug_weight is not None :
        raise NotImplementedError("get_edge_features is not implemented")
    else:
        Xve = X[hyperedge_index[0]] # [nnz, C]
        Xe = scatter(Xve, hyperedge_index[1], dim=0, reduce='mean') # [E, C]
        Xe = Xe * degE
...
class UniGCNIIEncoder(nn.Module):
    
    def __init__(self, in_dim, node_dim, drop_p = 0.5, num_layers=2, cached = False, nonlin_str='relu'):
        super(UniGCNIIEncoder, self).__init__()
        self.in_dim = in_dim
        self.node_dim = node_dim
        self.num_layers = num_layers
        self.act = select_activation(nonlin_str)
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor) :
        ...
        for i,conv in enumerate(self.convs[1:]) : 
            ...
            if i == len(self.convs[1:]) - 1 : 
                ...
                x = conv(x, hyperedge_index, alpha, beta, x0, degV, degE)
                x = self.act(x)
        return x, x0 # Only Returns Node Embeddings

class UniGCNIIDecoder(nn.Module):
    
    def __init__(self, in_dim, node_dim, drop_p = 0.5, num_layers=2, cached = False, 
                device = 'cuda:0', nonlin_str='relu'):
        super(UniGCNIIDecoder, self).__init__()
        self.in_dim = in_dim
        self.node_dim = node_dim
        self.num_layers = num_layers
        self.act = select_activation(nonlin_str)
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor) :
        ...
        for i,conv in enumerate(self.convs[1:]) : 
            ...
            if i == len(self.convs[1:]) - 1 : 
                ...
                x = self.act(conv(x, hyperedge_index, alpha, beta, x0, degV, degE))
...
class end2endNN(nn.Module) : # HNN Fine-Tuning
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor, num_nodes: int, num_edges: int) : 
        Z = self.encoder(x = x, hyperedge_index = hyperedge_index)
